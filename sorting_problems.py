...
def sort(list):
    for pos in range(len(list)):
        min_pos = pos
        for scanner in range(pos + 1, len(list)):
            if list[scanner] < list[min_pos]:
                min_pos = scanner
        list[pos], list[min_pos] = list[min_pos], list[pos]
    return(list)
sorted_list = sort(sort_me)
print(sorted_list)
...
def insertion_sort(list):
    for key_pos in range(1, len(list)):
        key = list[key_pos]
        scan_pos = key_pos - 1
        while scan_pos >= 0 and list[scan_pos] > key:
...
def insertion_sort(list):
    ...
    for key_pos in range(len(list)):
        ...
        small_loop += scan_pos
        list[scan_pos + 1] = key
    print("Large Loop Iterates", big_loop, "Times")
    print("Small Loop Iterates", small_loop, "Times")
    return(list)
